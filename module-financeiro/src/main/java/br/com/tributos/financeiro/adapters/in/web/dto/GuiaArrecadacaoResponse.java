...
public record GuiaArrecadacaoResponse(
    ...
    public static GuiaArrecadacaoResponse de(GuiaArrecadacao g) {
        return new GuiaArrecadacaoResponse(
            g.id(), g.numero(), g.tipoTributo(), g.origemTipo(), g.origemId(), g.contribuinteId(),
            g.competenciaMes(), g.competenciaAno(), g.dataEmissao(), g.dataVencimento(), g.valor(),
            g.situacao(), g.formaPagamentoId(), g.dataEfetivacao(), g.valorPago(), g.codigoBarras(),
            g.pixTxid(), g.descricaoAvulsa()
        );
    }
}
