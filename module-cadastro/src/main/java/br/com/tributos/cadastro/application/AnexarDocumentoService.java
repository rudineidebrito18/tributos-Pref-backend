...
@Service
public class AnexarDocumentoService {
    ...
    @Transactional
    public Documento executar(UUID pessoaId, String tipo, MultipartFile arquivo) {
        ...
        try (InputStream stream = arquivo.getInputStream()) {
            storageChave = armazenamentoArquivo.salvar(tenantId, pessoaId, documentoId, nomeArquivo, stream);
        } catch (IOException ex) {
            throw new ValidationException("Não foi possível ler o arquivo enviado.");
        }

        return documentoRepository.salvar(new Documento(
            documentoId, tenantId, pessoaId, tipo.trim(), null, null, nomeArquivo, conteudoTipo,
            arquivo.getSize(), storageChave, false, Instant.now()
        ));
    }

    private void validarPessoaExiste(UUID pessoaId) {
